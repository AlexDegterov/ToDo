...
export class CalendarComponent implements OnInit {
  ...
  private year: number;           // номер года в календаре
  private month: number;          // номер месяца в календаре
...
// tslint:disable-next-line: max-line-length
      this.arrDate.push ({day: this.i, dayW: this.i + 2 - this.firstDayWeekInMonth, holiday: (this.weekEnd.indexOf(this.i) < 0) ? false : true, today: ((this.i + 2 - this.firstDayWeekInMonth === this.day) && this.yearNow === this.year && this.monthNow === this.month) ? true : false});
    }
    return this.arrDate;
  }
